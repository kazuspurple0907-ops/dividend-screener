#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""高配当株スクリーニングアプリ - ステートレス版 (portfolio/api_key はクライアント管理)"""
...
app = Flask(__name__)
CORS(app)  # Coworkアーティファクト / ローカルファイルからのアクセスを許可
...
# ============================================================
# APIキー取得（リクエストボディ優先 → 環境変数フォールバック）
# ============================================================
def get_api_key_from_body(data=None):
    if data is None:
    ...
    if not key:
        raise ValueError('APIキーが必要です。設定タブで入力してください。')
    return key
...
# スクリーニング用バルク取得
# ============================================================
def jq_get_bulk(endpoint, api_key, params=None):
    headers = {'x-api-key': api_key}
    all_data = []
    p = dict(params or {})
    while True:
...
def get_master_all(api_key):
    def fetch():
        rows = jq_get_bulk('/equities/master', api_key)
        latest = {}
        for r in rows:
            code = r.get('Code', '')
...
def get_fins_all(api_key):
    ...
    try:
                ...
                try:
                    ...
                    if r.status_code == 429:
                        ...
                        if consecutive_429 >= 3:
                            wait_count += 1
                            if wait_count > 10:
                                flog(f'429バースト{wait_count}回目 → 中断')
                                break
                    ...
                    if r.status_code == 400 and not sub_end_detected:
                        m = re.search(r'(\d{4}-\d{2}-\d{2})\s*~\s*(\d{4}-\d{2}-\d{2})', r.text)
                        if m:
                            sub_end_str = m.group(2)
                            flog(f'サブスクリプション範囲: {m.group(1)} ~ {sub_end_str}')
        ...
        if len(all_fins) < 100:
            return all_fins
        with open(fins_v2_file, 'w', encoding='utf-8') as f:
...
# スクリーニング
# ============================================================
def run_screening(api_key, portfolio_codes=None):
    if portfolio_codes is None:
        portfolio_codes = set()
    bulk_master_file = os.path.join(CACHE_DIR, 'master_v2.json')
    fins_v2_file     = os.path.join(CACHE_DIR, 'fins_v2.json')
    if not os.path.exists(bulk_master_file) or not os.path.exists(fins_v2_file):
        return None
    try:
    ...
    for fname in os.listdir(CACHE_DIR):
        if fname.startswith('fn_') and fname.endswith('.json'):
            code4 = fname[3:-5]
            try:
                with open(os.path.join(CACHE_DIR, fname)) as fp:
...
def get_recommendations(portfolio_metrics, screen_results):
    ...
    for pm in portfolio_metrics:
        ...
        if not sector or not same_sec:
            recs.append({
                'current': current_info, 'current_annual_div': annual_div_now,
                'gain_pct': round(gain_pct, 1), 'yield_at_cost': round(yield_at_cost, 2),
                'sell_amount': round(sell_amount, 0), 'replacements': [],
                'gain_conditions': 0, 'no_same_sector': True,
            })
            continue
        candidates_raw = sorted(same_sec, key=lambda x: (x.get('conditions_met', 0), x.get('score', 0)), reverse=True)[:5]
        candidates = []
        for c in candidates_raw:
            ...
            cand = {k: c.get(k) for k in ['code','name','sector','dividend_yield','conditions_met','score','verdict','verdict_class','per','pbr','roe','payout_ratio','equity_ratio','market_cap_oku','current_price','dps']}
            cand.update({'est_shares': round(c_shares_est, 0), 'est_annual_div': c_annual_div, 'div_change': div_change, 'div_change_pct': div_change_pct})
        ...
        recs.append({
            'current': current_info, 'current_annual_div': annual_div_now,
            'gain_pct': round(gain_pct, 1), 'yield_at_cost': round(yield_at_cost, 2),
            'sell_amount': round(sell_amount, 0), 'replacements': better,
            'gain_conditions': best_met - pf_met, 'no_same_sector': False,
        })
    with_cands  = sorted([r for r in recs if not r.get('no_same_sector')], key=lambda x: x['gain_conditions'], reverse=True)
...
def parse_sbi_csv(raw_bytes):
    ...
    header_idx = next((i for i, l in enumerate(lines) if any(c in l for c in CODE_COLS)), None)
...
# ============================================================
# キャッシュから1銘柄の指標を組み立て（APIコールなし）
# ============================================================
def _build_metrics_from_cache(stock):
    code  = stock.get('code', '')
    code4 = code[:4]
    master, fins, rt = {}, {}, None
    for prefix, _ in [('ms', 'master'), ('fn', 'fins'), ('rt', 'rt')]:
...
def _do_portfolio_refresh(stocks, api_key):
    ...
    try:
        ...
        try:
            ...
            master_all = get_master_all(api_key)
            rlog(f'master bulk got {len(master_all)} entries')
            ms_ok = 0
            for code_key, info in master_all.items():
                code4 = code_key[:4]
                if code4 in target_set:
                    with open(os.path.join(CACHE_DIR, f'ms_{code4}.json'), 'w') as f:
        ...
        if ms_count == 0:
            for stock in stocks:
                ...
                if not c: continue
                stub = {'Code': c+'0','CoName': stock.get('name', c),'S17Nm': '','MktNm': ''}
                with open(os.path.join(CACHE_DIR, f'ms_{c}.json'), 'w') as f:
                    json.dump(stub, f, ensure_ascii=False)
            rlog(f'stub ms_ written: {len(stocks)} stocks')
        # fins
        fins_ok_jq = 0; fins_ok_yf = 0; consec_429 = 0; use_yf = False
        for c in target_codes:
            fn_path = os.path.join(CACHE_DIR, f'fn_{c}.json')
            if os.path.exists(fn_path) and (time.time() - os.path.getmtime(fn_path)) < 21600:
                fins_ok_jq += 1; continue
            jq_ok = False
            if not use_yf:
                code5 = c + '0'
                try:
            ...
            if not jq_ok:
                try:
                    ...
                    fins_yf = {
                        'Code': c+'0', 'DiscDate': datetime.now().strftime('%Y-%m-%d'),
                        'EPS': float(info.get('trailingEps') or 0),
                        'BPS': float(info.get('bookValue') or 0),
                        'Sales': float(info.get('totalRevenue') or 0),
                        'OP': float(info.get('operatingIncome') or 0),
                        'EqAR': float(info.get('totalStockholdersEquity') or 0) / float(info.get('totalAssets') or 1),
                        'DivAnn': float(info.get('dividendRate') or 0),
                        'FDivAnn': 0,
                        'PayoutRatioAnn': float(info.get('payoutRatio') or 0),
                        'ShOutFY': float(info.get('sharesOutstanding') or 0),
                        'Source': 'yfinance',
                    }
                    if fins_yf['EPS'] or fins_yf['BPS'] or fins_yf['DivAnn']:
                        with open(fn_path, 'w') as f:
        ...
        try:
            ...
            rt_count = sum(1 for c in target_codes if os.path.exists(os.path.join(CACHE_DIR, f'rt_{c}.json')))
            rlog(f'prices done: {rt_count}/{len(target_codes)}')
        except Exception as e:
            rlog(f'prices exc: {e}')
        # fins_v2 background
        fins_v2_file = os.path.join(CACHE_DIR, 'fins_v2.json')
            ...
            if fdiv_ann > 0 and fdiv_ann < div_ann:
                ...
                if os.path.exists(ms_path):
                    with open(ms_path) as f:
                        ms = json.load(f)
                    name = ms.get('CoName', '')
                cuts.append({'type': 'dividend_cut', 'code': code4, 'company': name,
                             'title': f'減配予想: 実績 {div_ann:.0f}円 → 来期予想 {fdiv_ann:.0f}円',
                             'time': '', 'source': 'キャッシュデータ', 'url': ''})
        except Exception:
            pass
    return cuts
...
def service_worker():
    sw = """
const CACHE = 'dividend-screener-v2';
...
def api_test():
    try:
        data = request.get_json(silent=True) or {}
        api_key = get_api_key_from_body(data)
        r = requests.get(f'{JQUANTS_V2}/equities/master',
                         headers={'x-api-key': api_key}, params={'code': '86970'}, timeout=10)
        r.raise_for_status()
...
def api_portfolio_metrics():
    """ポートフォリオ指標をキャッシュから即時返却。ポートフォリオはリクエストボディから受け取る。"""
    data      = request.get_json(silent=True) or {}
...
def api_portfolio_metrics_refresh():
    """バックグラウンドでキャッシュ再取得。portfolio + api_key をボディから受け取る。"""
    data      = request.get_json(silent=True) or {}
    portfolio = data.get('portfolio', [])
    if not portfolio:
    ...
    threading.Thread(target=_do_portfolio_refresh, args=(portfolio, api_key), daemon=True).start()
    return jsonify({'ok': True, 'message': 'バックグラウンドで更新開始', 'total': len(portfolio)}), 202

@app.route('/api/screen', methods=['POST', 'GET'])
def api_screen():
    try:
        data = request.get_json(silent=True) or {}
        portfolio_codes = set(data.get('portfolio_codes', []))
        try:
            api_key = get_api_key_from_body(data)
        except ValueError as e:
            return jsonify({'warning': str(e), 'data': []}), 400
        results = run_screening(api_key, portfolio_codes)
        if results is None:
            return jsonify({
                'building': True,
...
def api_screen_prefetch():
    ...
    try:
        ...
        return jsonify({'ok': False, 'message': 'APIキーが必要です'}), 400
    if os.path.exists(_BULK_LOCK_FILE) and (time.time() - os.path.getmtime(_BULK_LOCK_FILE)) < 3600:
        return jsonify({'ok': True, 'message': 'バックグラウンドで取得中です'})
    def do_fetch():
        try: get_master_all(api_key)
        except Exception: pass
        try: get_fins_all(api_key)
        except Exception: pass
    threading.Thread(target=do_fetch, daemon=True).start()
    return jsonify({'ok': True, 'message': 'バックグラウンドで取得開始しました'})
...
def api_recommendations():
    try:
        ...
        portfolio_codes = {s.get('code', '')[:4] for s in portfolio}
        pf_metrics = []
        for stock in portfolio:
            try:
                pf_metrics.append(_build_metrics_from_cache(stock))
            except Exception as e:
                pf_metrics.append({'code': stock.get('code',''), 'error': str(e), **stock})
        screen_results = run_screening(api_key, portfolio_codes) or []
        return jsonify(get_recommendations(pf_metrics, screen_results))
    except Exception as e:
        return jsonify({'error': str(e)}), 500

@app.route('/api/alerts', methods=['POST', 'GET'])
def api_alerts():
    data   = request.get_json(silent=True) or {}
    codes  = data.get('portfolio_codes', [])
    if not codes:
        return jsonify([])
...
def api_import_csv():
    try:
        ...
        if not result:
            return jsonify({'ok': False, 'message': '株式銘柄が見つかりませんでした'}), 400
        # サーバーには保存しない。クライアントが受け取ってlocalStorageへ保存する。
        return jsonify({'ok': True, 'imported': len(result), 'portfolio': result})
    except Exception as e:
...
if __name__ == '__main__':
    ...
    print('=' * 55)
    print(f'  高配当株スクリーニングアプリ (J-Quants v2 / stateless)')
